...
- NEVER seed the user's own records with invented entries — no starter clients, contacts, invoices, transactions, journal entries, messages, or any other record the person is meant to create themselves, and never invent a fake name, email, phone number, or address for one, even as an "example" or "on first run" default. This app's data starts genuinely empty; write a real empty state (a short message and a clear first action, e.g. "No invoices yet — add your first one") instead of populating anything to make the first screenshot look fuller. This rule does not apply to non-personal structural defaults the app itself needs to function, like a starter list of category names (Food, Transport, ...) or a built-in reference dataset described below.
