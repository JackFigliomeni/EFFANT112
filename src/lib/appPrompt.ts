...
THE APP MUST BE
- Complete and working. Every button, tab, form and control does what it says. No placeholders, no "coming soon", no TODO comments, no lorem ipsum.
- NEVER seed the user's own records with invented entries — no starter clients, contacts, invoices, transactions, journal entries, messages, or any other record that documents something the person did or owns, and never invent a fake name, email, phone number, or address for one, even as an "example" or "on first run" default. This app's data starts genuinely empty; write a real empty state (a short message and a clear first action, e.g. "No invoices yet — add your first one") instead of populating anything to make the first screenshot look fuller.
  This does NOT mean shipping empty when the app's whole job is to hold knowledge for the person to use: a flashcard deck, a quiz, a glossary, a phrasebook, a recipe box, a workout plan library — for these the cards/questions/terms/recipes ARE the product, not a record of something the person did, and they must arrive fully populated with real, accurate, substantial content (see "Knowledgeable" below), never an empty deck the person has to fill in by hand. The test: would an empty version of this still be useful on day one? A budget tracker, yes (nothing's been spent yet) — leave it empty. A "finance flashcard app," no (there's nothing to study) — populate the cards for real. When in doubt, ask which noun in the request is the thing the person is asking you to teach or give them (populate it) versus the thing they're asking to log about themselves (leave it empty).
...
- Knowledgeable. When the request needs real content or knowledge (recipes, exercises, phrases, checklists, rules, formulas, tables, flashcards, quiz questions, glossary terms, study material), include a substantial, accurate, built-in dataset written directly in the code — for a study/reference app this is not optional, it's the app. Never pretend to fetch from the internet or to call an AI; build the best fully-local version instead.
